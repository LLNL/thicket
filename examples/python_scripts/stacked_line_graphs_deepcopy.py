...
def process_thickets(input_files, groupby_parameter, filter_prefix, top_ten, output_graphs, additional_args):
    ...
    ctk_grouped_df = ctk.dataframe.groupby("name").sum()

    if filter_prefix != "":
        filter_df = copy.deepcopy(ctk_grouped_df.filter(like=filter_prefix, axis=0))
    else:
        filter_df = ctk_grouped_df

    if top_ten:
        filter_df = filter_df.nlargest(10, [(world_size[0], "Total time")])

    if "perc" in output_graphs:
        make_stacked_line_graph(filter_df, "perc", world_size, additional_args.graph_title, additional_args.graph_xlabel, "Percentage of Runtime" if additional_args.graph_ylabel == "no_label" else additional_args.graph_ylabel)
    if "total" in output_graphs:
        make_stacked_line_graph(filter_df, "Total time", world_size, additional_args.graph_title, additional_args.graph_xlabel, "Total Time" if additional_args.graph_ylabel == "no_label" else additional_args.graph_ylabel)
