...
var TreeTable = /*#__PURE__*/function () {
  function TreeTable(div, width, height, tree_model, table_model) {
    ...
    this.y_scale = linear_linear().range([layout.margins.top, height]).domain([0, Object.values(this.indented_tree).length]);
    this.pre_render();
  }
...
function setup(data) {
  var tree_model = new TreeModel(data.graph[0]);
  var table_model = null;
  var tree_max_w = window.innerWidth * .7;
  var tree_max_h = window.innerHeight * .9;
  var tree_div = src_select("#treetable");
  var treetable = new TreeTable(tree_div, tree_max_w, tree_max_h, tree_model, table_model); //Bind render to store updates
