...
def generate():
    th0 = write_profile(thicket.from_caliperreader, f"{OUT}t0.pstats", files[0])
    th1 = write_profile(thicket.from_caliperreader, f"{OUT}t1.pstats", files[1])
    th2 = write_profile(thicket.from_caliperreader, f"{OUT}t2.pstats", files[2])
    th3 = write_profile(thicket.from_caliperreader, f"{OUT}t3.pstats", files[3])
    th4 = write_profile(thicket.from_caliperreader, f"{OUT}t4.pstats", files[4])
    th5 = write_profile(thicket.from_caliperreader, f"{OUT}t5.pstats", files[5])
    th6 = write_profile(thicket.from_caliperreader, f"{OUT}t6.pstats", files[6])
    th7 = write_profile(thicket.from_caliperreader, f"{OUT}t7.pstats", files[7])
    th8 = write_profile(thicket.from_caliperreader, f"{OUT}t8.pstats", files[8])
    th9 = write_profile(thicket.from_caliperreader, f"{OUT}t9.pstats", files[9])
    th_list = [th0, th1, th2, th3, th4, th5, th6, th7, th8, th9]
    for th in th_list:
        print(len(th.graph))
    assert not all_equal([th.graph for th in th_list])
    t_ens = write_profile(
        thicket.unify_ensemble, f"{OUT}unify-ensemble.pstats", th_list
    )
    ...
    t_graph = write_profile(thicket.opt_graph_unify, f"{OUT}opt-union.pstats", th_list2)
