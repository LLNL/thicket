...
# Thicket Project Developers. See the top-level COPYRIGHT file for details.
#
...
def _all_thicket_files(root="."):
    """Generates root-relative paths of all files in the thicket repository."""
    visited = set()
...
def _licensed_files(root):
    for relpath in _all_thicket_files(root):
        if any(regex.match(relpath) for regex in licensed_files):
            yield relpath


def list_files(root):
    """list files in thicket that should have license headers"""
...
license_line_regexes = [
    r"Copyright (%d|%d) Lawrence Livermore National Security, LLC and other"
    % (latest_year - 1, latest_year),  # allow a little leeway: current or last year
    r"Thicket Project Developers\. See the top-level LICENSE file for details.",
...
def verify(root):
    """verify that files in thicket have the right license header"""
